...
public class GameDisplay extends ScreenAdapter {
    ...
    public GameDisplay(TicTacToe game) {
        ...
        recordLabelX.setFontScale(2f); // Increased font size for emphasis
        recordLabelX.setColor(Color.CYAN); // Change color to Cyan
        recordLabelX.setStyle(new Label.LabelStyle(recordLabelX.getStyle().font, Color.CYAN)); // Changing the style
        recordLabelX.setPosition(20, Gdx.graphics.getHeight() - 50);
        
        recordLabelO = new Label("O Record: " + game.getPlayer2().getRecord(), skin);
        recordLabelO.setFontScale(2f); // Increased font size for emphasis
        recordLabelO.setColor(Color.MAGENTA); // Change color to Magenta
        recordLabelO.setStyle(new Label.LabelStyle(recordLabelO.getStyle().font, Color.MAGENTA)); // Changing the style
        recordLabelO.setPosition(20, Gdx.graphics.getHeight() - 80);
        
        // Update the turn label similarly if needed
        turnLabel = new Label("Turn: " + game.getCurPlayerMark(), skin);
        turnLabel.setFontScale(2f); // Larger font size for the turn label
        turnLabel.setColor(Color.YELLOW); // Change color to Yellow
        turnLabel.setPosition(Gdx.graphics.getWidth() / 2f, Gdx.graphics.getHeight() - 100, Align.center);
        
        // Adding all the labels to the stage
